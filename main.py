...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith('lang_'):
        ...
        if os.path.exists(VIDEO_PATH):
            # Отправляем видео как документ, чтобы убрать вращающиеся часики
            with open(VIDEO_PATH, 'rb') as video_file:
                await context.bot.send_document(chat_id=update.effective_chat.id, document=video_file)
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith('lang_'):
        ...
        # Загрузка видео и обновление сообщения
        if os.path.exists(VIDEO_PATH):
            # Отправляем видео с отключенными уведомлениями
            with open(VIDEO_PATH, 'rb') as video_file:
